...
class Iq9075CandidateEvidenceWorkflowTest(unittest.TestCase):
    ...
    def test_publisher_is_standalone_manual_exact_tag_only(self) -> None:
        ...
        self.assertEqual(
            self.publisher.count("github.ref == 'refs/tags/candidate-publisher-v19'"),
            3,
        )
    ...
    def test_authorization_accepts_only_exact_v19_ref_and_workflow_identity(self) -> None:
        ...
        active_ref = "refs/tags/candidate-publisher-v19"
        cases = [
            (active_ref, active_ref, "16", True),
            ...
            (active_ref + "0", active_ref + "0", "16", False),
        ]
        for retired_ref in (
            ...
            cases.extend(
                (
                    (retired_ref, retired_ref, "16", False),
                    (retired_ref, active_ref, "16", False),
                    (active_ref, retired_ref, "16", False),
                )
            )
    ...
    def test_privileged_jobs_execute_only_signed_publisher_code(self) -> None:
        for name, job, environment in (
            ...
            with self.subTest(job=name):
                ...
                self.assertIn(
                    "refs/tags/candidate-publisher-v19", job
                )
                self.assertEqual(
                    OIDC.WORKFLOW_REF,
                    "plaid-ai/NUV-AGENT/.github/workflows/iq9075-candidate-trusted-publish.yml@refs/tags/candidate-publisher-v19",
                )
    ...
    def test_candidate_v19_prevents_downgrade_or_scope_expansion(self) -> None:
        self.assertGreaterEqual(self.publisher.count('= "0.1.121" ]'), 3)
        self.assertGreaterEqual(self.publisher.count('= "16" ]'), 3)
    ...
    def test_policy_and_runbook_name_the_same_trusted_tag(self) -> None:
        ...
        self.assertEqual(
            policy["candidatePublisher"],
            {
                ...
                "tagRef": "refs/tags/candidate-publisher-v19",
                "retiredTagRefs": [
                    "refs/tags/candidate-publisher-v1",
                    "refs/tags/candidate-publisher-v2",
                    "refs/tags/candidate-publisher-v3",
        ...
        self.assertIn("--ref candidate-publisher-v19", current_runbook)
    ...
    def test_accepts_signed_exact_standalone_claims(self) -> None:
        with tempfile.TemporaryDirectory() as raw_root:
            ...
            self.assertEqual(
                result["workflowRef"],
                "plaid-ai/NUV-AGENT/.github/workflows/"
                "iq9075-candidate-trusted-publish.yml@refs/tags/candidate-publisher-v19",
            )
        ...
        cases = (
            ...
            {"job_workflow_ref": OIDC.WORKFLOW_REF, "job_workflow_sha": "2" * 40},
            {
                "job_workflow_ref": OIDC.WORKFLOW_REF.replace(
                    "candidate-publisher-v19", "candidate-publisher-v2"
                ),
                "job_workflow_sha": "1" * 40,
            },
            {
                "job_workflow_ref": OIDC.WORKFLOW_REF.replace(
                    "candidate-publisher-v19", "candidate-publisher-v3"
                ),
                "job_workflow_sha": "1" * 40,
            },
            {
            ...
            {
                "job_workflow_ref": OIDC.WORKFLOW_REF.replace(
                    "candidate-publisher-v19", "candidate-publisher-v4"
                ),
                "job_workflow_sha": "1" * 40,
            },
        )
