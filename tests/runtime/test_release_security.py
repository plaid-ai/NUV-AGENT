...
class SequenceAndPromotionTest(unittest.TestCase):
    def test_sequence_skips_only_explicitly_retired_unpublished_candidates(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw_root:
            root = Path(raw_root)
            artifact = root / "nuv-agent_0.1.127_iq9075-aarch64.agent-bundle.tar.gz"
            ...
            with (
                ...
                reservation, output = PLAN_OTA.plan_sequence(
                    ...
                    version="0.1.127",
                    component_sha="a" * 40,
                    requested_sequence=29,
                    config_schema="12",
                    min_updater_version="0.2.0",
                    built_at="2026-09-02T00:00:00+00:00",
                )
                self.assertEqual(reservation["releaseSequence"], 29)
                self.assertEqual(output["latest_sequence"], "1")
                self.assertEqual(
                    output["reservation_object"], "releases/reservations/iq9075/29.json"
                )
                for rejected in (*range(1, 29), 30):
...
def candidate_publisher_tag_response(version: int = 32) -> dict[str, object]:
...
class CandidatePublisherTagEnvelopeTest(unittest.TestCase):
    ...
    def test_accepts_signed_api_message_shape_and_object_hash(self) -> None:
        ...
        self.assertEqual(
            tag_object["message"],
            "NUVION IQ9075 candidate publisher v32\n"
        ...
        for malformed in (None, "", signature.removesuffix("-----END PGP SIGNATURE-----\n")):
            with self.subTest(signature=malformed):
                ...
                if isinstance(malformed, str):
                    tag_object["message"] = "NUVION IQ9075 candidate publisher v32\n" + malformed
    ...
    def test_rejects_wrong_payload_headers_even_with_matching_raw_object_hash(self) -> None:
        cases = (
            ...
            ("tag candidate-publisher-v32", "tag candidate-publisher-v1"),
...
class SettingsPolicyTest(unittest.TestCase):
    def test_general_writers_require_hardened_review_with_pinned_admin_roster(
        ...
        for name, environment in policy["requiredEnvironments"].items():
            ...
            self.assertEqual(
                ...
                (
                    [{"name": "candidate-publisher-v32", "type": "tag"}]
                    if name
                    in {
                        "iq9075-candidate-sign",
                        "iq9075-candidate-stage",
    ...
    def test_settings_audit_accepts_no_environment_wait_and_rejects_reviewer_rule(
        ...
        responses: dict[str, object] = {
            ...
            "/repos/plaid-ai/NUV-AGENT/git/ref/tags/candidate-publisher-v32": {
                "ref": "refs/tags/candidate-publisher-v32",
        ...
        for name in (
            ...
            expected_deployment_policy = (
                {"id": 1, "name": "candidate-publisher-v32", "type": "tag"}
        ...
        with (
            ...
            mock.patch.object(
                ...
                return_value={
                    "candidate_publisher_tag": "candidate-publisher-v32",
                    "candidate_publisher_tag_ref": "refs/tags/candidate-publisher-v32",
