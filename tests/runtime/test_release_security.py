...
class SequenceAndPromotionTest(unittest.TestCase):
    def test_sequence_skips_only_explicitly_retired_unpublished_candidates(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as raw_root:
            root = Path(raw_root)
            artifact = root / "nuv-agent_0.1.125_iq9075-aarch64.agent-bundle.tar.gz"
            ...
            with (
                ...
                reservation, output = PLAN_OTA.plan_sequence(
                    ...
                    version="0.1.125",
                    component_sha="a" * 40,
                    requested_sequence=27,
                    config_schema="12",
                    min_updater_version="0.2.0",
                    built_at="2026-09-02T00:00:00+00:00",
                )
                self.assertEqual(reservation["releaseSequence"], 27)
                self.assertEqual(output["latest_sequence"], "1")
                self.assertEqual(
                    output["reservation_object"], "releases/reservations/iq9075/27.json"
                )
                for rejected in (*range(1, 27), 28):
...
def candidate_publisher_tag_response(version: int = 30) -> dict[str, object]:
...
class CandidatePublisherTagEnvelopeTest(unittest.TestCase):
    ...
    def test_accepts_signed_api_message_shape_and_object_hash(self) -> None:
        ...
        self.assertEqual(
            tag_object["message"],
            "NUVION IQ9075 candidate publisher v30\n"
        ...
        for malformed in (None, "", signature.removesuffix("-----END PGP SIGNATURE-----\n")):
            with self.subTest(signature=malformed):
                ...
                if isinstance(malformed, str):
                    tag_object["message"] = "NUVION IQ9075 candidate publisher v30\n" + malformed
    ...
    def test_rejects_wrong_payload_headers_even_with_matching_raw_object_hash(self) -> None:
        cases = (
            ...
            ("tag candidate-publisher-v30", "tag candidate-publisher-v1"),
...
class SettingsPolicyTest(unittest.TestCase):
    ...
    def test_settings_audit_accepts_no_environment_wait_and_rejects_reviewer_rule(
        ...
        responses: dict[str, object] = {
            ...
            "/repos/plaid-ai/NUV-AGENT/git/ref/tags/candidate-publisher-v30": {
                "ref": "refs/tags/candidate-publisher-v30",
        ...
        for name in (
            ...
            expected_deployment_policy = (
                {"id": 1, "name": "candidate-publisher-v30", "type": "tag"}
        ...
        with (
            ...
            mock.patch.object(
                ...
                return_value={
                    "candidate_publisher_tag": "candidate-publisher-v30",
                    "candidate_publisher_tag_ref": "refs/tags/candidate-publisher-v30",
