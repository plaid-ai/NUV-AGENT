...
class CandidatePublisherTagTest(unittest.TestCase):
    ...
    def test_accepts_v9_signed_with_new_key_frozen_in_publisher(self) -> None:
        repository, publisher_sha, component_sha, policy, signers = self._fixture()
        result = self._verify(
            repository, publisher_sha, component_sha, policy, signers
        )
        self.assertEqual(result["candidate_publisher_tag"], "candidate-publisher-v9")
        self.assertEqual(
            result["candidate_publisher_tag_ref"], "refs/tags/candidate-publisher-v9"
        )
    ...
    def test_rejects_policy_drift(self) -> None:
        ...
        for field, value in (
            ...
            ("retiredTagRefs", ["refs/tags/candidate-publisher-v9"]),
            (
                "retiredTagRefs",
