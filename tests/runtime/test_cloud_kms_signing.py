...
class OtaKmsFederationTest(unittest.TestCase):
    def test_each_provider_pins_workflow_sha_subject_ref_actor_and_repository(self):
        ...
        self.assertEqual(set(plan), {"candidate-v23", "release-main-v23"})
        for name, config in plan.items():
            ...
            self.assertIn("refs/tags/candidate-publisher-v23" if name == "candidate-v23" else "refs/heads/main", condition)
        with self.assertRaises(ValueError):
            module.plan("main")
