...
Current publisher: `candidate-publisher-v40`, Agent `0.1.135`, release sequence
`37`, schema `12`, minimum updater `0.2.0`. Sequences 2–36 and publishers v1–v39
...
1. Merge the reviewed publisher, verify its complete CI gate, and record the
   ...
   gh workflow run kms-approve-release.yml --repo plaid-ai/NUV-AGENT --ref main \
     -f target_sha="$P" -f tag_name=candidate-publisher-v40 \
     -f tag_message='NUVION IQ9075 candidate publisher v40'
   ```

2. Verify the annotated tag's object, commit, and KMS OpenPGP signature. Add v40
   ...
   existing candidate-sign/stage tag policies from v39 to v40 and verify them.
...
4. Dispatch the immutable candidate (no device update command is issued):
   ...
   ```sh
     ...
     --repo plaid-ai/NUV-AGENT --ref candidate-publisher-v40 \
     -f component_sha="$P" -f version=0.1.135 -f release_sequence=37
   ```
...
This cutover does not mark `0.1.135` READY. Physical automatic rollback and commit
...
Publisher v36/sequence 33 produced a signed `0.1.131` candidate and completed
automatic rollback and normal commit on IQ9075. The following config/stream
qualification required local retry-only runner changes after the immutable
publisher had been tagged. Those bytes were not part of publisher v36, so the
result cannot be signed as formal readiness evidence. Sequence 33 remains an
immutable development rollback baseline.

Publisher v37/sequence 34 produced signed `0.1.132` evidence and passed the
automatic rollback scenario. Its commit scenario exposed a systemd start-rate
...
## Candidate v21 sampled streaming observation retry

Sequence17 passed physical rollback and commit. Its CONFIG_APPLY change and
restore and STREAM_POLICY command all succeeded, but the verification reader
rejected a later healthy hysteresis observation because it expected the
transient `policy_activated` reason. This attempt remains non-promotable.

The reader and independent release verifier now recognize later healthy
controller observations while still requiring the exact signed command,
contiguous command sequence, acknowledged observations, increasing revisions,
actual bounded bitrate decrease and recovery, continuous stream health, and
drained queues. Runtime controller behavior is unchanged. Publisher v21 uses
